...
SINGULAR service from the `service_factory`, by doing it this way the controller
only knows which methods it can call in SINGULAR Service but nothing about the
database.
"""
...
async def get_PLURAL(page_size: pydantic.conint(ge=1, le=100) = 20,
    ...
    """Get a paginated list of PLURAL. TODO(Add Doc)
    Args:
...
async def add_SINGULAR(SINGULAR: SINGULAR_schemas.Create,
                       service=fastapi.Depends(
                           service_factory.get_SINGULAR_services)):
    """Create a new SINGULAR. TODO(Add Doc)
    Args:
...
async def update_SINGULAR(PRIMARY_KEY_NAME: PRIMARY_KEY_TYPE,
                          ...
                          service=fastapi.Depends(
                              service_factory.get_SINGULAR_services)):
    """Updates an existing SINGULAR. TODO(Add Doc and fix exception text)
    Args:
...
async def get_SINGULAR(PRIMARY_KEY_NAME: PRIMARY_KEY_TYPE,
                       service=fastapi.Depends(
                           service_factory.get_SINGULAR_services)):
    """Get a SINGULAR with the provided PRIMARY_KEY_NAME. TODO(Add Doc)
    Args:
        PRIMARY_KEY_NAME (PRIMARY_KEY_TYPE): PRIMARY_KEY_NAME
    """
...
async def delete_SINGULAR(PRIMARY_KEY_NAME: PRIMARY_KEY_TYPE,
                          service=fastapi.Depends(
                              service_factory.get_SINGULAR_services)):
    """Deletes the SINGULAR that belongs to the provided PRIMARY_KEY_NAME. TODO(Add Doc)
    Args:
        PRIMARY_KEY_NAME (PRIMARY_KEY_TYPE): PRIMARY_KEY_NAME
    """
